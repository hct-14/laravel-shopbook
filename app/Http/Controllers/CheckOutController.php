...
class CheckOutController extends Controller
{
    ...
    public function Save_Checkout(Request $request) {
        ...
        $data = [
            'user_id' => $UserId,
            'product_id' => implode(',', $productIds), // Combine product_ids into a comma-separated string
    ...
    foreach ($checkthongtin as $order) {
        ...
        if (isset($groupedProducts[$productId])) {
            $groupedProducts[$productId]['product_titles'][] = $order->product_title;
        } else {
            $groupedProducts[$productId] = [
                'product_titles' => [$order->product_title],
    ...
    // dd($checkthongtin);
        return view('category.order_finall', compact('brand_product', 'categories', 'checkthongtin'));
    }
